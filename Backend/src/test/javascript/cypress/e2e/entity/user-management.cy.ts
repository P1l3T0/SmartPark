...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Kenny_Parker60', email: 'Alexander15@hotmail.com' };
  ...
  describe.skip('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Jayne_Feest');
      cy.get(`[data-cy="login"]`).should('have.value', 'Jayne_Feest');

      cy.get(`[data-cy="firstName"]`).type('Roxanne');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Roxanne');

      cy.get(`[data-cy="lastName"]`).type('Lowe-Hodkiewicz');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Lowe-Hodkiewicz');

      cy.get(`[data-cy="email"]`).type('Gerardo6@hotmail.com');
      cy.get(`[data-cy="email"]`).should('have.value', 'Gerardo6@hotmail.com');
