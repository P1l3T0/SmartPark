...
import static com.tusofia.smartpark.domain.BookingAsserts.*;
...
import com.tusofia.smartpark.repository.BookingRepository;
import com.tusofia.smartpark.service.dto.BookingDTO;
import com.tusofia.smartpark.service.mapper.BookingMapper;
...
class BookingResourceIT {
    ...
    private static final BookingStatus DEFAULT_STATUS = BookingStatus.CONFIRMED;
    ...
    public static Booking createEntity(EntityManager em) {
        ...
        if (TestUtil.findAll(em, ParkingSpot.class).isEmpty()) {
            parkingSpot = ParkingSpotResourceIT.createEntity();
    ...
    public static Booking createUpdatedEntity(EntityManager em) {
        ...
        updatedBooking.setUserProfile(userProfile);
        // Add required entity
        Vehicle vehicle;
        if (TestUtil.findAll(em, Vehicle.class).isEmpty()) {
        ...
        updatedBooking.setVehicle(vehicle);
        // Add required entity
        ParkingSpot parkingSpot;
        if (TestUtil.findAll(em, ParkingSpot.class).isEmpty()) {
            parkingSpot = ParkingSpotResourceIT.createUpdatedEntity();
            em.persist(parkingSpot);
            em.flush();
        } else {
            parkingSpot = TestUtil.findAll(em, ParkingSpot.class).get(0);
        }
        updatedBooking.setParkingSpot(parkingSpot);
    ...
    void putExistingBooking() throws Exception {
        ...
        updatedBooking.startDate(UPDATED_START_DATE).endDate(UPDATED_END_DATE).dateCreated(UPDATED_DATE_CREATED).status(UPDATED_STATUS);
